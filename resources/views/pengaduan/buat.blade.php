...
@section('meta_description', 'Sampaikan keluhan layanan air Anda ke PDAM Tirtanadi Cabang Padang Bulan tanpa perlu
    akun.')
...
@section('content')
    <section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-16">
        ...
        <div class="mb-8 sm:mb-10">
            <span
                class="inline-flex items-center gap-2 bg-brand-blue/10 text-brand-blue text-xs font-semibold px-4 py-1.5 rounded-full">
        ...
        @if (session('success'))
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 sm:p-12 text-center">
                <div
                    class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-brand-green/10 flex items-center justify-center mx-auto mb-6">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#3FA75B"
                        stroke-width="2.5">
                        <path d="M20 6L9 17l-5-5" />
                    </svg>
                </div>
                <h2 class="font-display font-bold text-xl sm:text-2xl text-ink">Pengaduan Berhasil Dikirim!</h2>
                <p class="text-slate-500 mt-2 text-sm sm:text-base">Simpan nomor pengaduan Anda untuk memantau
                    perkembangannya.</p>
                ...
                <div class="mt-6 inline-flex flex-col items-center bg-brand-blue/5 rounded-2xl px-6 sm:px-10 py-5 sm:py-6">
                    ...
                    <p class="font-display font-bold text-xl sm:text-2xl text-brand-blue tracking-wide mt-1">
                        {{ session('success') }}</p>
                </div>

                <div class="flex flex-col xs:flex-row justify-center gap-3 mt-8">
                    <a href="/lacak"
                        class="inline-flex items-center justify-center gap-2 bg-brand-blue text-white font-semibold rounded-xl px-6 py-3 shadow-lg shadow-brand-blue/30 hover:bg-brand-bluelight transition text-sm">
                        🔍 Lacak Pengaduan Ini
                    </a>
                    <a href="/pengaduan/buat"
                        class="inline-flex items-center justify-center gap-2 border-2 border-slate-200 text-ink font-semibold rounded-xl px-6 py-3 hover:border-brand-blue hover:text-brand-blue transition text-sm">
                        Buat Pengaduan Lain
                    </a>
                </div>
            </div>
            ...
            <div id="form-pengaduan" x-data="pengaduanForm()">
                        ...
                        <div class="flex items-center {{ $i < count($stepLabels) - 1 ? 'flex-1' : '' }}">
                            <div class="flex flex-col items-center shrink-0">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center font-display font-bold text-sm transition-all duration-300"
                                    :class="currentStep > {{ $stepNum }} ? 'bg-brand-green text-white' : (currentStep ===
                                        {{ $stepNum }} ? 'bg-brand-blue text-white ring-4 ring-brand-blue/20' :
                                        'bg-slate-100 text-slate-400')">
                                    <span x-show="currentStep <= {{ $stepNum }}">{{ $stepNum }}</span>
                                    <svg x-show="currentStep > {{ $stepNum }}" width="16" height="16"
                                        viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"
                                        style="display:none">
                                        <path d="M20 6L9 17l-5-5" />
                                    </svg>
                                </div>
                                <p class="text-xs font-medium mt-2 text-center whitespace-nowrap"
                                    :class="currentStep === {{ $stepNum }} ? 'text-brand-blue' : 'text-slate-400'">
                                    {{ $label }}</p>
                            </div>
                            @if ($i < count($stepLabels) - 1)
                                <div class="flex-1 h-0.5 mx-2 mb-6 transition-colors duration-300"
                                    :class="currentStep > {{ $stepNum }} ? 'bg-brand-green' : 'bg-slate-100'"></div>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                <div class="sm:hidden mb-6">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-semibold text-slate-500">Langkah <span x-text="currentStep"></span> dari 4
                        </p>
                ...
                <form method="POST" action="/pengaduan" enctype="multipart/form-data"
                    class="bg-white rounded-3xl border border-slate-100 shadow-sm p-5 sm:p-8 lg:p-10"
                    ...
                    <div data-step="1" x-show="currentStep === 1">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4 sm:gap-5">
                            <div class="sm:col-span-2">
                                <label class="block text-sm font-medium text-ink mb-1.5">Nama Lengkap <span
                                ...
                                <input type="text" name="nama_pelapor" x-model="form.nama_pelapor" maxlength="255"
                                    ...
                                    :class="touched.nama_pelapor && errors.nama_pelapor ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full h-12 rounded-xl border px-4 text-sm outline-none transition focus:ring-2">
                                <p x-show="touched.nama_pelapor && errors.nama_pelapor" x-text="errors.nama_pelapor"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-ink mb-1.5">Nomor HP / WhatsApp <span
                                        class="text-red-500">*</span></label>
                                <input type="tel" name="no_hp" x-model="form.no_hp" maxlength="20"
                                    @blur="touched.no_hp = true; validateField('no_hp')" placeholder="08xxxxxxxxxx"
                                    :class="touched.no_hp && errors.no_hp ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full h-12 rounded-xl border px-4 text-sm outline-none transition focus:ring-2">
                                <p x-show="touched.no_hp && errors.no_hp" x-text="errors.no_hp"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="no_pelanggan" x-model="form.no_pelanggan" maxlength="50"
                                    ...
                                    :class="touched.no_pelanggan && errors.no_pelanggan ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full h-12 rounded-xl border px-4 text-sm outline-none transition focus:ring-2">
                                <p x-show="touched.no_pelanggan && errors.no_pelanggan" x-text="errors.no_pelanggan"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="email" name="email" x-model="form.email" maxlength="255"
                                    @blur="touched.email = true; validateField('email')" placeholder="nama@email.com"
                                    :class="touched.email && errors.email ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full h-12 rounded-xl border px-4 text-sm outline-none transition focus:ring-2">
                                <p x-show="touched.email && errors.email" x-text="errors.email"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>

                            <div class="sm:col-span-2">
                                <label class="block text-sm font-medium text-ink mb-1.5">Alamat Lengkap <span
                                        class="text-red-500">*</span></label>
                                <textarea name="alamat" x-model="form.alamat" rows="3" @blur="touched.alamat = true; validateField('alamat')"
                                    placeholder="Jalan, nomor rumah, kelurahan, kecamatan"
                                    :class="touched.alamat && errors.alamat ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full rounded-xl border px-4 py-3 text-sm outline-none transition focus:ring-2 resize-none"></textarea>
                                <p x-show="touched.alamat && errors.alamat" x-text="errors.alamat"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="no_rumah_patokan" x-model="form.no_rumah_patokan"
                                    maxlength="255" placeholder="Contoh: seberang minimarket, dekat gapura RT 5"
                    ...
                    <div data-step="2" x-show="currentStep === 2" style="display:none">
                        ...
                        <div class="grid gap-4 sm:gap-5">
                            <div>
                                <label class="block text-sm font-medium text-ink mb-1.5">Kategori Pengaduan <span
                                        class="text-red-500">*</span></label>
                                <select name="kategori_pengaduan_id" x-model="form.kategori_pengaduan_id"
                                    ...
                                    :class="touched.kategori_pengaduan_id && errors.kategori_pengaduan_id ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                ...
                                <p x-show="touched.kategori_pengaduan_id && errors.kategori_pengaduan_id"
                                    x-text="errors.kategori_pengaduan_id" class="text-red-500 text-xs mt-1.5"
                                    style="display:none"></p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-ink mb-1.5">Judul Pengaduan <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="judul" x-model="form.judul" maxlength="255"
                                    ...
                                    :class="touched.judul && errors.judul ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full h-12 rounded-xl border px-4 text-sm outline-none transition focus:ring-2">
                                <p x-show="touched.judul && errors.judul" x-text="errors.judul"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-ink mb-1.5">Deskripsi Lengkap <span
                                        class="text-red-500">*</span></label>
                                <textarea name="deskripsi" x-model="form.deskripsi" rows="5"
                                    ...
                                    :class="touched.deskripsi && errors.deskripsi ?
                                        'border-red-400 focus:ring-red-400 focus:border-red-400' :
                                        'border-slate-200 focus:ring-brand-blue focus:border-brand-blue'"
                                    class="w-full rounded-xl border px-4 py-3 text-sm outline-none transition focus:ring-2 resize-none"></textarea>
                                <p x-show="touched.deskripsi && errors.deskripsi" x-text="errors.deskripsi"
                                    class="text-red-500 text-xs mt-1.5" style="display:none"></p>
                                <p x-show="!(touched.deskripsi && errors.deskripsi)"
                                    class="text-xs text-slate-400 mt-1.5">Semakin detail, semakin cepat petugas memahami
                                    masalah Anda.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div data-step="3" x-show="currentStep === 3" style="display:none">
                        ...
                        <div class="grid gap-4 sm:gap-5">
                            <div>
                                <label class="block text-sm font-medium text-ink mb-1.5">
                                    Detail Lokasi Kejadian <span class="text-slate-400 font-normal text-xs">—
                                        opsional</span>
                                </label>
                            ...
                            <div>
                                ...
                                <div class="border-2 border-dashed rounded-2xl p-6 sm:p-8 text-center transition-colors"
                                    :class="errors.foto ? 'border-red-300 bg-red-50' : (dragging ?
                                        'border-brand-blue bg-brand-blue/5' : 'border-slate-200')"
                                    ...
                                    <template x-if="!fotoPreview">
                                        <div>
                                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none"
                                                stroke="#94A3B8" stroke-width="1.5" class="mx-auto mb-3">
                                    ...
                                    <input type="file" name="foto" x-ref="fotoInput"
                                        accept="image/jpeg,image/jpg,image/png" class="hidden"
                                        @change="handleFoto($event.target.files[0])">
                                </div>
                                <p x-show="errors.foto" x-text="errors.foto" class="text-red-500 text-xs mt-2"
                                    style="display:none"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div data-step="4" x-show="currentStep === 4" style="display:none">
                        ...
                        <div class="space-y-4">
                            <div class="bg-slate-50 rounded-2xl p-4 sm:p-5">
                                ...
                                <dl class="grid sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    <div>
                                        ...
                                        <dd class="text-ink font-medium" x-text="form.nama_pelapor || 'Belum diisi'"></dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-400 text-xs">No. HP</dt>
                                        <dd class="text-ink font-medium" x-text="form.no_hp || 'Belum diisi'"></dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-400 text-xs">NPA</dt>
                                        <dd class="text-ink font-medium" x-text="form.no_pelanggan || 'Tidak diisi'"></dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-400 text-xs">Email</dt>
                                        <dd class="text-ink font-medium" x-text="form.email || 'Tidak diisi'"></dd>
                                    </div>
                                    <div class="sm:col-span-2">
                                        <dt class="text-slate-400 text-xs">Alamat</dt>
                                        <dd class="text-ink font-medium" x-text="form.alamat || 'Belum diisi'"></dd>
                                    </div>
                                    <div class="sm:col-span-2">
                                        <dt class="text-slate-400 text-xs">Patokan Rumah</dt>
                                        <dd class="text-ink font-medium" x-text="form.no_rumah_patokan || 'Tidak diisi'">
                                        </dd>
                                    </div>
                                </dl>
                            </div>
                            ...
                            <div class="bg-slate-50 rounded-2xl p-4 sm:p-5">
                                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Detail
                                    Pengaduan</p>
                                <dl class="space-y-2 text-sm">
                                    <div>
                                        <dt class="text-slate-400 text-xs">Kategori</dt>
                                        <dd class="text-ink font-medium" x-text="categoryLabel || 'Belum dipilih'"></dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-400 text-xs">Judul</dt>
                                        <dd class="text-ink font-medium" x-text="form.judul || 'Belum diisi'"></dd>
                                    </div>
                                    <div>
                                        <dt class="text-slate-400 text-xs">Deskripsi</dt>
                                        <dd class="text-ink font-medium" x-text="form.deskripsi || 'Belum diisi'"></dd>
                                    </div>
                                </dl>
                            </div>

                            <div class="bg-slate-50 rounded-2xl p-4 sm:p-5">
                                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Lokasi & Foto
                                </p>
                                ...
                                <p x-show="!fotoPreview" class="text-slate-400 text-xs italic" style="display:none">Tidak
                                    ada foto dilampirkan</p>
                            </div>

                            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-amber-700 text-sm"
                                x-show="!isFormValid" style="display:none">
                                ⚠️ Masih ada data wajib yang belum lengkap atau belum valid. Silakan cek kembali Langkah 1
                                dan 2.
                            </div>

                            <div class="flex items-start gap-3 bg-brand-blue/5 rounded-2xl p-4">
                                <input type="checkbox" id="konfirmasi" x-model="konfirmasi"
                                    class="mt-0.5 w-4 h-4 accent-brand-blue">
                                <label for="konfirmasi" class="text-sm text-slate-600">
                                    Saya menyatakan bahwa data dan informasi yang saya isi di atas benar dan dapat
                    ...
                    <div class="flex items-center justify-between mt-8 pt-6 border-t border-slate-100">
                        ...
                        <button type="submit" x-show="currentStep === 4" style="display:none" :disabled="!konfirmasi"
                            :class="konfirmasi ?
                                'bg-brand-green hover:opacity-90 shadow-lg shadow-brand-green/30 cursor-pointer' :
            ...
            return {
                ...
                rules: {
                    ...
                    no_hp: {
                        required: true,
                        pattern: /^[0-9+]{9,15}$/,
                        patternMsg: 'Nomor HP harus angka, 9-15 digit.'
                    },
                    no_pelanggan: {
                        required: false,
                        pattern: /^[0-9]*$/,
                        patternMsg: 'Nomor Pelanggan hanya boleh berisi angka.'
                    },
                    email: {
                        required: false,
                        pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                    ...
                    alamat: {
                        required: true,
                        minLength: 10,
                        minLengthMsg: 'Alamat terlalu singkat, minimal 10 karakter.'
                    },
                    ...
                    judul: {
                        ...
                        minLength: 5,
                        maxLength: 255,
                        minLengthMsg: 'Judul minimal 5 karakter.'
                    },
                    deskripsi: {
                        required: true,
                        minLength: 20,
                        minLengthMsg: 'Ceritakan lebih detail lagi, minimal 20 karakter.'
                    },
                },
                ...
                submitForm(event) {
                    ...
                    if (!step1Valid || !step2Valid || this.errors.foto || !this.konfirmasi) {
                        ...
                        if (!step1Valid) {
                            this.currentStep = 1;
                            this.focusFirstError(1);
                        } else if (!step2Valid) {
                            this.currentStep = 2;
                            this.focusFirstError(2);
                        } else {
                            this.currentStep = 4;
                            this.scrollToTop();
                        }
